...
async function listSources(token, org, repo, path = '') {
  ...
  // API returns direct array, although the documentation says it returns an object with sources property
...
export async function run() {
  try {
    ...
    for (const source of sources) {
      // Skip reserved items
      if (reservedItems.some(reserved => source.name.toLowerCase().includes(reserved.toLowerCase()))) {
      ...
      try {
        ...
        const destPath = `/${org}/${repo}/${backup.path}/${source.name}`;
        
        await moveSource(token, source.path, destPath);
        core.info(`Moved ${source.name} to ${backup.name}`);
        movedCount++;
        
      } catch (err) {
        core.warning(`Failed to move ${source.name} to ${backup.name}: ${err.message}`);
      }
    }
